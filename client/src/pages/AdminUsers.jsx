...
function AdminUsers() {
  ...
  const handleUserClick = async (u) => {
    ...
    try {
      ...
      setSelectedDocs(res.data.data.documents || []);
    } catch {
      setSelectedDocs([]);
    } finally {
  ...
  return (
    <div className="page-wide">
      ...
      <div className="flex gap-2 mb-4 flex-wrap">
        <input
          className="flex-1 min-w-[200px] px-3 py-2 text-sm border border-border rounded-sm bg-white text-charcoal placeholder:text-stone-light focus:outline-none focus:border-pink"
          placeholder="Search by name or email..."
          value={search}
          onChange={(e) => { setSearch(e.target.value); setPage(1); }}
        />
        <select
          className="px-3 py-2 text-sm border border-border rounded-sm bg-white text-charcoal focus:outline-none focus:border-pink"
          value={roleFilter}
      ...
      {loading ? (
        ...
        <div className="card overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody>
              {users.map((u) => (
                <tr key={u.id} className="border-t border-border hover:bg-pink-subtle/40 cursor-pointer transition" onClick={() => handleUserClick(u)}>
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2">
                      <Avatar name={u.name} size="sm" src={getFileUrl(u.profilePhoto)} />
                      <span className="font-medium text-plum">{u.name}</span>
                    </div>
                  </td>
                  ...
                  <td className="px-4 py-3 text-right">
                    {u.role !== 'admin' && (
                      <button
                        ...
                        onClick={(e) => { e.stopPropagation(); handleSuspend(u.id, u.name); }}
                      >
      ...
      {pages > 1 && (
        <div className="flex items-center justify-center gap-2 mt-4">
          <button
            className="px-3 py-1.5 text-sm border border-border rounded-sm bg-white text-stone hover:border-pink hover:text-pink disabled:opacity-30 cursor-pointer transition"
            disabled={page <= 1}
        ...
        <>
          {modalLoading ? (
            <div className="fixed inset-0 z-[9998] flex items-center justify-center bg-black/40 backdrop-blur-sm">
              <div className="bg-white rounded px-8 py-6 shadow-xl">
                <p className="text-sm text-stone">Loading user details...</p>
              </div>
            </div>
          ) : (
            <UserDetailModal
